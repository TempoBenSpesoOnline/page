 ...
 */
  body: function(name) {
    ...
    Input.shared.list.foreach((el) => {
      if (el.cat.indexOf(name) != -1) {elements += el}
    });
    elements.map((el) => {
      return `<li id="${el.ref}"><a href="${el.href}">${el.text}</a>${el.warn ? `<span class="btn btn-outline-warning">${el.warn}</span>` : `` }${el.info ? `<span class="btn btn-outline-info">${el.info}</span>` : ``}<span class="btn invisible">Se vedi questo, allora probabilmente non riescia vedere neanche il resto del sito.</span></li>`;
    });
...
fs.writeFile("museo/index.html", utils.head("Musei") + utils.body("museo"), (err) => {
  if (err) throw err;
  console.log("museo done")
})

fs.writeFile("divertimento/index.html", utils.head("Divertimenti"), (err) => {
  if (err) throw err;
  console.log("museo done")
})